...
async function saveSessions(sessions) {
  try {
    await window.storage.set(STORAGE_KEY, JSON.stringify(sessions));
...
function AnalyticsView({ sessions, onExport }) {
  ...
  // Determine "regulars": players with >= 2 sessions, sorted by total sessions desc
  const regulars = useMemo(() => {
    return allPlayerNames
      .filter(n => playerStats[n].sessions >= 2)
      .sort((a, b) => playerStats[b].sessions - playerStats[a].sessions)
      .slice(0, 9); // cap at 9 lines for readability
  }, [allPlayerNames, playerStats]);

  // Cumulative winnings data
  // For each session (chronological), for each regular:
  //   if they played: cumulative += their profit that session
  //   if they didn't play: cumulative stays flat (carry forward)
  const cumulativeData = useMemo(() => {
    ...
    ended.forEach((s, i) => {
      ...
      s.players.forEach(p => {
        if (p.cashout === null) return;
        sessionPlayers.add(p.name);
        if (cumulative[p.name] !== undefined) {
          const buyin = p.buyins.reduce((a, x) => a + x, 0);
          cumulative[p.name] += (p.cashout - buyin);
        }
      });
      // Players who didn't play this session: cumulative stays the same (flat line)
      ...
      regulars.forEach(n => {
        point[n] = Math.round(cumulative[n] * 100) / 100;
      });
      data.push(point);
    });
    return data;
  ...
  // Detailed stats table sorted by net profit
...
const S = {
  ...
  sectionTitle: { fontSize: 12, fontWeight: 600, color: "#6b7280", textTransform: "uppercase", letterSpacing: 1, marginBottom: 10, margin: 0, marginBottom: 10 },
...
if (typeof document !== "undefined") {
  ...
  el.textContent = `@keyframes spin{to{transform:rotate(360deg)}} *{box-sizing:border-box} body{margin:0} .recharts-surface{overflow:visible}`;
